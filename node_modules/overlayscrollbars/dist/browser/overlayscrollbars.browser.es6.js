...
const OverlayScrollbarsGlobal = function(t) {
  "use strict";
  function each(t, n) {
    if (isArrayLike(t)) {
  ...
  const createStructureSetupElements = t => {
    ...
    const {W: B, J: V, Z: Y, K: j, tt: q, nt: G} = R;
    const F = [];
    ...
    const appendElements = () => {
      ...
      if (G) {
        insertBefore(j, G);
        push(F, removeElements.bind(0, G));
      }
      if (M) {
        const t = attr(j, Ot);
        attr(j, Ot, "-1");
    ...
    return [ R, appendElements, runEachAndClear.bind(0, F) ];
  };
  ...
  const {round: qt, max: Gt, sign: Ft} = Math;
  ...
  OverlayScrollbars.env = () => {
    const {k: t, I: n, A: o, B: s, V: e, L: c, N: r, X: l, j: i, q: a, G: u, F: d} = getEnvironment();
    return assignDeep({}, {
      scrollbarsSize: t,
